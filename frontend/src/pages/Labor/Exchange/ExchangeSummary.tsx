...
const ExchangeSummary = () => {
  ...
  const handleConfirmRecalculation = async () => {
    try {
      // Đã tính toán và lưu rồi, chỉ cần đóng modal và refresh data
      toast.success('Đã cập nhật số dư công thành công!');
      setShowRecalculateModal(false);
      fetchSummary(); // Tải lại dữ liệu
    } catch (err) {
  ...
  const handleAction = async () => {
    if (exchanges.length === 0) {
      // Trường hợp chưa có dữ liệu - tạo dữ liệu mới
      await handleInitializeData();
    } else {
      // Trường hợp đã có dữ liệu - tính lại
  ...
  return (
    <div className="container mx-auto p-4">
      <div className="flex justify-between items-center mb-6">
        <div>
          <h1 className="text-2xl font-bold">Tổng quan số dư đổi công</h1>
      ...
      {/* Modal hiển thị kết quả tính toán lại */}
