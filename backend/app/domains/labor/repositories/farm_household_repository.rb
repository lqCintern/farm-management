  ...
  module Repositories
    class FarmHouseholdRepository
      ...
      def create(household_entity)
        record = ::Labor::FarmHousehold.new(
      ...
      def update(household_entity)
        record = ::Labor::FarmHousehold.find_by(id: household_entity.id)
      ...
      def get_summary(id)
        ...
        active_workers = ::Labor::HouseholdWorker.where(household_id: id, is_active: true).count
        total_exchanges = ::Labor::LaborExchange.where("household_a_id = ? OR household_b_id = ?", id, id).count
        pending_requests = ::Labor::LaborRequest.where(requesting_household_id: id, status: :pending).count
      ...
      def add_worker(household_id, worker_id, params)
        household_record = ::Labor::FarmHousehold.find_by(id: household_id)
