  ...
  module V1
    module SupplyChain
      class FarmerSupplyOrdersController < BaseController
        ...
        def index
          result = CleanArch.farmer_list_orders.execute(
        ...
        def show
          result = CleanArch.farmer_get_order_details.execute(
            params[:id],
            current_user.user_id
          )
        ...
        def create
          result = CleanArch.farmer_create_order.execute(
            current_user.user_id,
        ...
        def update
          result = CleanArch.farmer_update_order.execute(
            params[:id],
            current_user.user_id,
            supply_order_params
          )
        ...
        def cancel
          result = CleanArch.farmer_cancel_order.execute(
            params[:id],
            current_user.user_id
          )
        ...
        def complete
          result = CleanArch.farmer_complete_order.execute(
            params[:id],
            current_user.user_id
          )
