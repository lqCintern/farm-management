module Repositories
  module Labor
    class HouseholdWorkerRepository
      include ::Interfaces::Repositories::Labor::HouseholdWorkerRepositoryInterface

      def find(id)
        record = ::Models::Labor::HouseholdWorker.find_by(id: id)
        return nil unless record
        map_to_entity(record)
      end

      def find_by_household(household_id)
        records = ::Models::Labor::HouseholdWorker.where(household_id: household_id)
                                        .includes(:worker)
                                        .active

        records.map do |record|
          worker_profile = ::Models::Labor::WorkerProfile.find_by(user_id: record.worker_id)
          {
            id: record.id,
      ...
      def create(household_worker_entity)
        record = ::Models::Labor::HouseholdWorker.new(
        ...
        ActiveRecord::Base.transaction do
          if record.save
            ...
            unless ::Labor::WorkerProfile.exists?(user_id: record.worker_id)
              profile = ::Models::Labor::WorkerProfile.new(user_id: record.worker_id)
              unless profile.save
      ...
      def update(household_worker_entity)
        record = ::Models::Labor::HouseholdWorker.find_by(id: household_worker_entity.id)
