module Repositories
  module Labor
    class LaborRequestRepository
      include ::Interfaces::Repositories::Labor::LaborRequestRepositoryInterface

      def find(id)
        record = ::Models::Labor::LaborRequest
      ...
      def find_for_household(household_id, filters = {})
        ...
        if filters[:exclude_joined] == true
          joined_groups = ::Models::Labor::LaborRequest.where(providing_household_id: household_id)
      ...
      def find_public_requests(filters = {})
        query = ::Models::Labor::LaborRequest.where(is_public: true, status: :pending)
      ...
      def find_related_requests(request_id)
        request = ::Models::Labor::LaborRequest.find_by(id: request_id)
        return [] unless request

        if request.parent_request_id.present?
          # Find siblings (other requests with the same parent)
          related = ::Models::Labor::LaborRequest.where(parent_request_id: request.parent_request_id)
                                      .where.not(id: request_id)
        elsif request.request_group_id.present?
          # Find children (requests that have this request as parent)
          related = ::Models::Labor::LaborRequest.where(parent_request_id: request_id)
        else
          related = ::Models::Labor::LaborRequest.none
        end
      ...
      def find_by_farm_activity(household_id, farm_activity_id)
        records = ::Models::Labor::LaborRequest.where(farm_activity_id: farm_activity_id)
      ...
      def update(id, attributes)
        record = ::Models::Labor::LaborRequest.find_by(id: id)
      ...
      def get_group_status(request_id)
        ...
        group_requests = ::Models::Labor::LaborRequest.where(request_group_id: request.request_group_id)
      ...
      def can_household_join_request(request_id, household_id)
        ...
        if request.max_acceptors.present?
          ...
          accepted_count = ::Models::Labor::LaborRequest.where(
        ...
        existing = ::Models::Labor::LaborRequest.exists?(
          request_group_id: request.request_group_id,
      ...
      def update_parent_status(child_request_id)
        child_request = ::Models::Labor::LaborRequest.find_by(id: child_request_id)
        return nil unless child_request&.parent_request_id

        # Find the parent request
        parent_id = child_request.parent_request_id
        parent_request = ::Models::Labor::LaborRequest.find_by(id: parent_id)
        return nil unless parent_request

        # Get all siblings (including the child request itself)
        siblings = ::Models::Labor::LaborRequest.where(parent_request_id: parent_id)
