module Repositories
  module Labor
    class WorkerProfileRepository
      include WorkerProfileRepositoryInterface
      
      def find(id)
        record = ::Labor::WorkerProfile.find_by(id: id)
        return nil unless record
        map_to_entity(record)
      end
      
      def find_by_user_id(user_id)
        record = ::Labor::WorkerProfile.find_by(user_id: user_id)
        return nil unless record
        map_to_entity(record)
      end
      ...
      def create(worker_profile_entity)
        record = ::Labor::WorkerProfile.find_by(user_id: worker_profile_entity.user_id)
        ...
        if record
          ...
          if record.save
            return { success: true, profile: map_to_entity(record), action: :updated }
          else
            return { success: false, errors: record.errors.full_messages }
          end
        else
          # Create new record
          record = ::Labor::WorkerProfile.new(
          ...
          if record.save
            return { success: true, profile: map_to_entity(record), action: :created }
          else
            return { success: false, errors: record.errors.full_messages }
          end
        end
      ...
      def get_statistics(user_id)
        profile = ::Labor::WorkerProfile.find_by(user_id: user_id)
        return nil unless profile

        completed_assignments = ::Labor::LaborAssignment.where(worker_id: user_id, status: :completed)
