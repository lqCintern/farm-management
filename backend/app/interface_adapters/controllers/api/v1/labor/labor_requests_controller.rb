      ...
      class LaborRequestsController < BaseController
        ...
        def index
          ...
          if current_household
            requests = Services::CleanArch.labor_list_requests.execute(current_household.id, filters)
        ...
        # Tạo yêu cầu thường
        def create
          result = Services::CleanArch.labor_create_request.execute(
          ...
          if result[:success]
            ...
            if result[:request].providing_household_id.present?
              notification_dto = Services::CleanArch.prepare_labor_notification.execute(result[:request])
        ...
        def create_mixed
          ...
          result = Services::CleanArch.labor_create_mixed_request.execute(
            current_household.id,
            labor_request_params.to_h,
            provider_ids,
            options
        ...
        def join
          result = Services::CleanArch.labor_join_request.execute(
            params[:id],
            current_household.id,
            current_user.id
          )
        ...
        def group_status
          result = Services::CleanArch.labor_get_group_status.execute(params[:id])
        ...
        def public_requests
          ...
          requests = Services::CleanArch.labor_list_public_requests.execute(filters)
          render_success_response(requests)
        end

        # Cập nhật yêu cầu
        def update
          result = Services::CleanArch.labor_update_request.execute(
            params[:id],
            labor_request_params.to_h,
            current_user.id
          )
        ...
        # Xóa yêu cầu
        def destroy
          result = Services::CleanArch.labor_delete_request.execute(params[:id], current_user.id)
        ...
        def suggest_workers
          ...
          result = Services::CleanArch.labor_suggest_workers.execute(
            params[:id],
            max_suggestions
          )
        ...
        def for_activity
          ...
          result = Services::CleanArch.labor_list_requests_by_activity.execute(
            current_household.id,
        ...
        def set_labor_request
          result = Services::CleanArch.labor_get_request.execute(params[:id])
