    ...
    module Labor
      class WorkerProfilesController < BaseController
        ...
        def index
          ...
          profiles = Services::CleanArch.labor_find_available_workers.execute(filters)
          render_success_response(profiles)
        end

        def show
          # Lấy thông tin chi tiết về worker bao gồm cả thống kê
          stats = Services::CleanArch.labor_get_worker_statistics.execute(@worker_profile.user)
          render_success_response(stats)
        end

        def create
          result = Services::CleanArch.labor_create_or_update_profile.execute(
            current_user,
        ...
        def my_profile
          result = Services::CleanArch.labor_get_or_create_profile.execute(current_user)

          if result[:success]
            stats = Services::CleanArch.labor_get_worker_statistics.execute(current_user)
            render_success_response(stats)
          else
            render_error_response(result[:errors], :unprocessable_entity)
          end
        ...
        def available_workers
          filters = params.permit(:skills, :max_daily_rate, :max_hourly_rate, :exclude_household_id).to_h
          workers = Services::CleanArch.labor_find_available_workers.execute(filters)
        ...
        def worker_availability
          ...
          result = Services::CleanArch.labor_check_worker_availability.execute(
            worker_id,
            start_date,
            end_date
          )
        ...
        def set_worker_profile
          @worker_profile = ::Models::Labor::WorkerProfile.find(params[:id])
        end
