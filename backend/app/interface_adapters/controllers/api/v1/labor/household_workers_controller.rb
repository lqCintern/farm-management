    ...
    module Labor
      class HouseholdWorkersController < BaseController
        ...
        def index
          ...
          workers = Services::CleanArch.labor_list_household_workers.execute(household_id)
        ...
        def create
          ...
          result = Services::CleanArch.labor_add_worker_to_household.execute(
            @household.id,
            worker_user.id,
            worker_params.to_h
          )
        ...
        def destroy
          result = Services::CleanArch.labor_remove_worker.execute(
            params[:id],
            current_user
          )
        ...
        def update_status
          ...
          result = Services::CleanArch.labor_update_worker_status.execute(
            params[:id],
            is_active,
            current_user
          )
        ...
        def set_household
          @household = ::Models::Labor::FarmHousehold.find(params[:farm_household_id]) if params[:farm_household_id]
        end

        def set_household_worker
          @household_worker = ::Models::Labor::HouseholdWorker.find(params[:id])
