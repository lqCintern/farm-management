  ...
  module V1
    module Labor
      class LaborAssignmentsController < BaseController
        ...
        def index
          assignments = Services::CleanArch.labor_list_request_assignments.execute(@labor_request.id)
        ...
        def batch_assign
          ...
          result = Services::CleanArch.labor_batch_assign_workers.execute(
            @labor_request,
            worker_ids,
            date_range,
            current_household
        ...
        def destroy
          result = Services::CleanArch.labor_delete_assignment.execute(params[:id], current_user)
        ...
        def report_completion
          ...
          result = Services::CleanArch.labor_update_assignment_status.execute(
            @labor_assignment,
            :worker_reported,
            { notes: notes },
            current_user
        ...
        def rate_worker
          result = Services::CleanArch.labor_rate_assignment.rate_worker(
            params[:id],
            params[:rating],
            current_user
          )
        ...
        def rate_farmer
          result = Services::CleanArch.labor_rate_assignment.rate_farmer(
            params[:id],
            params[:rating],
            current_user
          )
        ...
        def my_assignments
          ...
          assignments = Services::CleanArch.labor_list_worker_assignments.execute(
            current_user.id,
            filters
          )
        ...
        def check_conflicts
          ...
          result = Services::CleanArch.labor_check_scheduling.check_conflicts(
            worker_id,
            date,
            start_time,
            end_time
        ...
        def worker_availability
          ...
          result = Services::CleanArch.labor_check_scheduling.get_worker_availability(
            worker_id,
            start_date,
            end_date
          )
        ...
        def household_assignments
          ...
          assignments = Services::CleanArch.labor_list_household_assignments.execute(
        ...
        def complete_multiple
          ...
          results = Services::CleanArch.labor_complete_multiple_assignments.execute(
            assignment_ids,
            { notes: notes },
            current_user
          )
        ...
        def set_labor_request
          ...
          @labor_request = ::Models::Labor::LaborRequest.find(request_id)
        rescue ActiveRecord::RecordNotFound
          render_error_response("Không tìm thấy yêu cầu lao động", :not_found)
        end

        def set_labor_assignment
          @labor_assignment = Services::CleanArch.labor_get_assignment.execute(params[:id])
