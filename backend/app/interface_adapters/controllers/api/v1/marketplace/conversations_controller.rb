  ...
  module V1
    module Marketplace
      class ConversationsController < BaseController
        ...
        def index
          conversations = ::Models::Conversation.for_user(current_user.user_id)
          ...
          render json: {
            ...
            unread_count: Models::Message.joins(:conversation)
        ...
        def messages
          messages = Services::Firebase::FirebaseMessageService.get_messages(@conversation.id, params[:limit] || 20)

          Services::Firebase::FirebaseMessageService.mark_all_as_read(@conversation.id, current_user.user_id)
        ...
        def create
          ...
          conversation = ::Models::Conversation.find_or_initialize_by(
          ...
          if !conversation.persisted?
            existing_conversation = ::Models::Conversation.find_by(
              product_listing: product_listing,
              sender: other_user,
          ...
          # Thêm tin nhắn đầu tiên bằng Services::Firebase::FirebaseMessageService
          if params[:message].present?
            Rails.logger.info "Saving first message to conversation #{conversation.id} using Services::Firebase::FirebaseMessageService"

            message_id = Services::Firebase::FirebaseMessageService.save_message(conversation.id, {
              user_id: current_user.user_id,
        ...
        def add_message
          ...
          # Lưu tin nhắn với Services::Firebase::FirebaseMessageService
          Rails.logger.info "Adding #{message_data[:type]} message to conversation #{@conversation.id}"

          message_id = Services::Firebase::FirebaseMessageService.save_message(@conversation.id, message_data)
        ...
        def set_conversation
          @conversation = ::Models::Conversation.find_by(id: params[:id])
