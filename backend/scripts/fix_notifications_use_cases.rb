...
class NotificationsUseCaseFixer
  def initialize
  ...
  def fix_all_use_cases
    puts "Starting to fix use cases in Notifications domain..."
    
    # Tìm tất cả các file use case trong thư mục notifications
    Dir.glob(@domains_dir.join('notifications/use_cases/*.rb')).each do |file_path|
  ...
  def fix_use_case_file(file_path)
    ...
    if content.include?("module Notifications\n  class")
...
# Chạy script
fixer = NotificationsUseCaseFixer.new
